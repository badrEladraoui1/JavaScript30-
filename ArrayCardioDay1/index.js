...
const sortingBirthdays = (inventors) => {
...
const years = (inventors) => {
    const count = inventors.reduce((accumulator, currentInventor) => {
        return accumulator + (currentInventor.passed - currentInventor.year)
    }, 0)
    return count
}
console.log(years(INVENTORS))

// 5. Sort the inventors by years lived

const yearsLived = (inventors) => {
    const sortedArr = inventors.sort(
        (inventorA, inventorB) => (inventorA.passed - inventorA.year) - (inventorB.passed - inventorB.year)
    )
    return sortedArr
}
